...
Функционал: Проверка отображения failed-теста
  Предыстория:
  ...
  Сценарий: тест
    ...
    Тогда ответ "response_grt" содержит значение jsonpath "ErrCode", соответствующее 1
    ...
    Тогда ответ "response_login" содержит значение jsonpath "ErrCode", соответствующее 0
    К тому же ответ "response_login" содержит значение jsonpath "Success", соответствующее true
    К тому же ответ "response_getCards" содержит список карт размером "1"
